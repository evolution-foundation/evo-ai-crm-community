...
class Facebook::Moderation::ActionExecutorService
  ...
  def build_message_attributes(conversation, content, comment_id)
    ...
    # Use the AgentBot as sender when the reply came from one. Otherwise the approval has no
    # author to credit: the reply is the account's, not any particular operator's.
    sender = agent_bot
