...
class Facebook::Moderation::ActionExecutorService
  ...
  def build_message_attributes(conversation, content, comment_id)
    ...
    # The bot owns the reply it drafted; otherwise the operator who approved it does
    # (approve! persists moderated_by before executing). An authorless reply would read
    # as "never answered" everywhere human_response? is consulted.
    sender = agent_bot || moderation.moderated_by
