...
class Contact < ApplicationRecord
  ...
  def self.resolved_contacts
    ...
    # EXISTS instead of a join: a join yields one row per contact_inbox, duplicating contacts and inflating counts
    where(
      "contacts.email <> '' OR contacts.phone_number <> '' OR contacts.identifier <> '' OR " \
      'EXISTS (SELECT 1 FROM contact_inboxes WHERE contact_inboxes.contact_id = contacts.id)'
    )
  end
