...
- **CRM-579 — a menção em conversa foi aposentada.** O recurso nunca teve produtor neste fork: nenhuma tela, em nenhum repo, jamais emitiu a marcação `mention://`, e o único leitor (`Messages::MentionService`) exigia id **numérico** (`\d+`) enquanto `users.id` e `teams.id` são uuid desde a migration inicial — os dois ramos da marcação, usuário e time, eram inalcançáveis desde o primeiro release. A decisão de aposentar em vez de consertar está registrada no card: corrigir só o id não bastava, porque o `MENTION_REGEX` casava o nome com `[\w_. ]+` e `\w` é ASCII em Ruby, então num produto em português a versão "consertada" produziria o pior caso — **notificar e ainda vazar a marcação crua** para o Slack e para o e-mail. Saem: a tabela `mentions` (a migration de drop tem `down`, que recria a tabela e os 3 índices — mas **não** repõe as notificações órfãs que o `up` apagou), `Mention`, `Messages::MentionService`, `Conversations::UserMentionJob`, o template `conversation_mention.liquid`, o tipo de notificação `conversation_mention`, o `MENTION_REGEX`, o evento `CONVERSATION_MENTIONED` e as 6 traduções do título. ⚠️ **Efeito observável:** o toggle *"Mencionado em conversa"* some da tela de Perfil — ele é derivado de `Notification::NOTIFICATION_TYPES` pelo `NotificationSettingSerializer`, então a tela deixa de oferecê-lo sozinha, sem mudança no front. A preferência já salva de cada usuário **não é tocada**: o número do enum é a *posição do bit* no FlagShihTzu, a chave sai e o bit 4 fica órfão e inerte, exatamente como os buracos 6–19 que a lista já tinha. Renumerar os tipos restantes é que deslocaria silenciosamente a preferência de todo mundo, e por isso **não foi feito**. Junto sai a exceção do `NotificationBuilder` que deixava passar notificação de conversa com contato bloqueado — ela existia só para a menção. O `transform_user_mention_content` virou `message_body_content`: perdeu a limpeza da marcação e manteve o `nil → ''`, que é load-bearing para mensagem de anexo sem texto. ⚠️ **O saneamento da marcação some do caminho de e-mail e de Slack** (`ConversationDrop`, `MessageDrop`, `SendOnSlackService`): dado nascido neste fork nunca casou o regex, mas uma instalação que importou mensagens de um Chatwoot upstream (id numérico + nome ASCII, o único par que o regex antigo limpava) passa a encaminhar a marcação crua. A UI do CRM já a mostrava crua — ela nunca esteve no caminho REST nem no do websocket. ⚠️ **Operação:** a limpeza das notificações órfãs do `up` é uma DML comum e, portanto, sujeita a RLS. Onde houver row-level security ativa para o papel que migra (é o caso do overlay enterprise, que põe FORCE RLS em `notifications`), o `DELETE` só alcança as linhas que o papel enxerga — zero, sem contexto de tenant. A migration agora detecta isso com `row_security_active()` e emite um WARNING nomeando as tabelas filtradas, em vez de reportar sucesso.

### Fixed

- **CRM-696** — `GET /api/v1/pipelines/by_contact/:contact_id` e `GET /api/v1/pipelines/by_conversation/:conversation_id` deixam de carregar todos os cards dos funis do contato para depois filtrar o dele: só os cards do contato ou da conversa são carregados, os contadores das etapas saem de uma leitura por funil e as não lidas e a última mensagem vêm em lote. Medido num funil de 5.000 cards: de ~0,5s para ~0,04s. ⚠️ **Mudança de contrato:** o `services_info.total_value` de cada funil passa a ser o total do funil inteiro, como no `GET /api/v1/pipelines/:id`; antes somava só os cards do contato, em desacordo com o `item_count` e com os totais das etapas do mesmo objeto. O valor dos cards do contato continua no `services_info` de cada card. Cada etapa ganha também `active_item_count` e `active_total_value`.
- **CRM-658** — Contatos de grupo (`type=group`) continuam fora da listagem e da busca por padrão, porque a tela de contatos depende disso, e o acesso pela API fica documentado: `GET /api/v1/contacts?include_groups=true` traz os grupos junto, `?type=group` traz só os grupos e `GET /api/v1/contacts/search?q=...&include_groups=true` os encontra na busca. O `POST`/`PUT /api/v1/contacts` que esbarrava num grupo escondido respondia só "has already been taken", e uma integração que sincroniza pela leitura concluía que o grupo não existia e tentava recriá-lo. Agora, em conflito de `identifier`, `email`, `phone_number` ou `tax_id`, o item do campo em `error.details` traz `existing_contact: { id, type }` para o cliente atualizar o registro existente. O `existing_contact` só aparece para quem tem `contacts.read` ou chama com service token.
- **CRM-665** — A paginação de mensagens de uma conversa deixa de perder mensagens gravadas no mesmo segundo do cursor. Os cursores `before`/`after` comparavam `created_at` de forma estrita, e o timestamp vem do provedor com resolução de segundo (`Time.zone.at(message_timestamp)`), então numa rajada de mídia toda mensagem que dividia o segundo do cursor ficava fora das duas páginas — para uma integração, indistinguível de perda de mensagem. A comparação passa a ser `(created_at, id)` e toda ordenação desempata por `id`. Medido por HTTP com 50 mensagens em 3 segundos: o walk com `before` devolvia 37 de 50 e passa a devolver 50 de 50. Junto, o parâmetro `page`, que era silenciosamente ignorado (respondia sempre as 20 mais novas), passa a ser honrado quando vem sem cursor — `page=1` é o bloco mais novo, `page=N` o enésimo bloco para trás. ⚠️ **Mudança de contrato:** `page` junto de `before`/`after`, `page` que não seja inteiro positivo, ou `page` acima de `MessageFinder::MAX_PAGE` (1.000.000, o teto em que o `OFFSET` ainda cabe no bigint do Postgres) respondem **422 `INVALID_PARAMETER`** em vez de serem ignorados. Nenhum consumidor interno envia `page` nesse endpoint (o widget e a API pública montam os parâmetros do finder por conta própria e o front só manda `before`/`after`); integração externa que mandava `page` junto do cursor e era atendida mesmo assim passa a receber o 422. ⚠️ **Resíduo declarado:** como `messages.id` é `gen_random_uuid()`, o critério de desempate é aleatório e não a ordem de chegada — uma rajada no mesmo segundo passa a ser exibida embaralhada (antes saía na ordem inversa da chegada), e um cliente que faz *tail* com `after` ainda perde parte da rajada. O conserto de fundo é gravar `created_at` com resolução abaixo de segundo na ingestão, fora do escopo deste card.
- **CRM-607** — Criar conversas em paralelo deixa de responder 500. `Conversation#ensure_display_id` alocava o `display_id` com `maximum(:display_id) + 1`, um read-modify-write sem trava: sob READ COMMITTED duas criações simultâneas leem o mesmo máximo (nenhuma enxerga a linha ainda não commitada da outra), escolhem o mesmo número e todas menos uma morrem no índice único `index_conversations_on_display_id`. Medido em Postgres 14: de 8 criações simultâneas, 1 gravava e 7 levantavam `ActiveRecord::RecordNotUnique`; com um `pg_advisory_xact_lock` antes do MAX, as 8 gravam 8 `display_id` distintos e 0 erros. No widget o estrago passava do 500: `Api::V1::Widget::ConversationsController#create` grava conversa e primeira mensagem na MESMA transação, e só trata `PreChatFormValidator::ValidationError`, então a violação abortava as duas e a primeira mensagem do visitante sumia junto com a conversa. O máximo continua saindo de um cálculo em Ruby, e não de uma sequence, porque é ele que dá a cada conta a numeração própria dela sob RLS na camada enterprise. ⚠️ **Efeito observável:** a alocação passou a ser um ponto de serialização. O lock é o transacional, liberado no COMMIT da transação que cria a conversa, então criações simultâneas agora esperam em vez de falhar, e uma transação longa que cria conversa faz as demais esperarem por ela; passando do `statement_timeout` (14s no default do `config/database.yml`), a espera termina em `PG::QueryCanceled` no lugar do `ActiveRecord::RecordNotUnique` de antes.
- **CRM-608** — Erro de validação da API passa a sair no idioma da instalação. Numa instalação pt-BR o 422 chegava em inglês, e desde o PR #387 do front é a mensagem do backend que a tela mostra. A causa não era a env `DEFAULT_LOCALE` faltando: `ActionController::Rescue` envolve `AbstractController::Callbacks`, então quando o `rescue_from` monta o corpo o `I18n.with_locale` do `around_action` já desempilhou e só resta o `I18n.default_locale` — setar apenas a env, que é o que o `around_action` lê, deixava a resposta em inglês do mesmo jeito (medido com actionpack 7.1.5.1). O `DEFAULT_LOCALE` passa a alimentar também o `I18n.default_locale`, em `config/initializers/languages.rb` e via `Rails.configuration`, porque o railtie do i18n aplica a configuração com a checagem de available_locales desligada; o default do repo continua `:en`. Junto disso: o `pt_BR.yml` ganhou as 28 chaves de `errors.messages` e o bloco `activerecord` que ActiveModel e ActiveRecord definem (sem elas a cadeia de fallback descrita abaixo serviria inglês em vez de pt-BR, e antes deste card a chave ausente virava o marcador "Translation missing"); o literal `'Validation failed'` do envelope, que é justamente o campo que o front lê para montar o toast, virou chave de tradução com o texto em `en` idêntico ao anterior; e o validador genérico de comprimento do `ApplicationRecord` passou de frase em inglês montada na mão para `:too_long` com `count`, porque ActiveModel devolve String verbatim sem passar pelo I18n. Corrige também o aninhamento de `errors.contacts.email.invalid` e `errors.contacts.phone_number.invalid` no `en.yml` e no `pt_BR.yml`, que estavam um nível fundo demais (sob `errors.conversations`) e faziam as duas validações de formato do `Contact` resolverem para "Translation missing" nesses dois idiomas — `es`, `fr`, `it` e `pt` já estavam corretos. ⚠️ **Efeito observável:** instalação com `DEFAULT_LOCALE=pt_BR` passa a responder os 422 de validação em português, inclusive o `error.message` do envelope, que vira "Falha na validação". Instalação sem a env continua respondendo os 422 em inglês; o que muda para ela é o caminho `?locale=` do widget, em que uma chave sem tradução no idioma pedido passa a sair em inglês em vez do marcador. O prefixo de atributo que o `errors.format` põe em cada `full_messages` também sai em pt-BR: o `pt_BR.yml` ganhou um bloco `attributes` na raiz, que vale para todo modelo, com as palavras do próprio formulário de contato (`name` → "Nome", `phone_number` → "Telefone", `identifier` → "Identificador", `tax_id` → "CPF/CNPJ"). `email` e `website` não entram porque o produto já os escreve assim — a linha do e-mail é "Email já está em uso", e não "E-mail já está em uso" como o card exemplificou. Atributo sem entrada continua com o nome da coluna. Efeito colateral medido, do mesmo `default_locale`: o `ApplicationMailer` tem `around_action :switch_locale` e esse `switch_locale` lê o `I18n.default_locale`, então o assunto do e-mail de resposta de conversa também passa a sair no idioma da instalação (`conversations.reply.email_subject` vira "Novas mensagens nesta conversa" em vez de "New messages on this conversation"). Vale para o serviço `sidekiq`, que é onde os `MailDeliveryJob` rodam, e por isso a env precisa estar nos dois. **Fallback de idioma, agora em todos os ambientes.** Alimentar o `I18n.default_locale` expõe um segundo defeito, mais largo que o 422: `config.i18n.fallbacks` só existia em `config/environments/production.rb`, e quatro dos seis idiomas habilitados (`es`, `fr`, `it`, `pt`) não têm `errors.messages` nem `errors.api` em catálogo (nem há gem `rails-i18n` no `Gemfile` para supri-los). Medido em `RAILS_ENV=test` com `DEFAULT_LOCALE=es`: o `error.message` virava `"Translation missing: es.errors.api.validation_failed"` e cada `details[].messages` virava o dump de cinco linhas `"Translation missing. Options considered were: ..."`, justamente no campo que o toast do front mostra. A cadeia saiu de `production.rb` e passou para `config/application.rb`, valendo para todo ambiente, fixada em `:en`, que é o único catálogo completo. Duas razões medidas para essa forma: `fallbacks = true` seria inócuo aqui, porque a cadeia default termina em `I18n.default_locale`, exatamente o valor que o `DEFAULT_LOCALE` move (medido: `I18n.fallbacks[:es] == [:es]`, sem `:en` dentro); e a linha antiga de produção, `[I18n.default_locale]`, já resolvia para `[:en]`, porque o railtie do i18n aplica a configuração em `after_initialize`, muito depois de o arquivo de ambiente ser lido, então produção fica com o mesmo comportamento. Com a cadeia no lugar, uma instalação `es` responde em espanhol as chaves que o `es.yml` tem e em inglês as que faltam: medido, `errors.validations.presence` sai "no debe estar en blanco" onde antes da mudança de `default_locale` saía "must not be blank", e `taken` volta a "has already been taken" em vez do marcador. **Duas saídas alternativas foram medidas e recusadas.** Restringir `i18n.available_locales` aos idiomas com catálogo de erros completo desliga `es`, `fr`, `it` e `pt` do `SwitchLocale#validate_and_get_locale`, que é quem valida `?locale=`: abandonaria 190 chaves traduzidas em cada um de `es`/`fr`/`it`, 192 em `pt` e 43 por idioma nos `devise.*.yml` para cobrir 28 chaves ausentes, e contradiria o `ApplicationHelper::SUPPORTED_LOCALES`, que lista os seis como o conjunto que o produto oferece. Restringir apenas quais valores de `DEFAULT_LOCALE` podem virar `default_locale` daria a quatro dos seis idiomas um rebaixamento silencioso para inglês, e dependeria de uma lista de "catálogo completo" que envelhece no instante em que alguém traduzir o `es.yml`. ⚠️ **Custo da escolha:** chave ausente deixa de se anunciar. O marcador `Translation missing` era o que tornava a lacuna visível em `dev` e `test`; agora ela renderiza o texto em inglês e parece correta, então falha de catálogo em `es`/`fr`/`it`/`pt` só aparece em diff de catálogo. Ligar `config.i18n.raise_on_missing_translations` fora de produção compensaria isso, e é decisão separada deste card. A cadeia medida é de dois passos por idioma, o idioma antes do inglês e nada no meio: `[:es, :en]`, `[:fr, :en]`, `[:it, :en]`, `[:pt, :en]`, `[:pt_BR, :en]` e `[:en]`. Em particular o `pt_BR` não passa a consultar o `pt`, porque o símbolo do locale usa sublinhado e a ancestralidade do `I18n::Locale::Fallbacks` quebra em hífen. **Declarado, não corrigido (vira card de seguimento).** O envelope de validação é montado à mão em **18 arquivos** de `app/controllers/`; este card traduziu **1**, o `api/base_controller.rb`. Restam **29 literais `'Validation failed'` em 17 controllers** (contagem medida nos dois lados: 30 ocorrências em 18 arquivos em `39e7fb6^`, 29 em 17 no HEAD), entre eles etiqueta (`labels_controller`), time (`teams_controller`), macro (`macros_controller`), funil (`pipelines_controller`, `pipeline_stages_controller`, `pipeline_service_definitions_controller`) e produto (`products_controller`, `products/variants_controller`). Nesses caminhos o `error.message` continua em inglês numa instalação pt-BR, e é justamente o campo que o toast mostra; o `details[]` deles já sai traduzido, porque vem de `format_validation_errors` sobre os erros do ActiveModel. Junto vai o outro seguimento: traduzir `errors.messages` e `errors.api.validation_failed` nos catálogos `es`, `fr`, `it` e `pt` (116 strings), que é o conserto de fundo para esses idiomas e não caberia aqui. **A env deixou de ser só o locale da request, e um consumidor não a lê como locale.** `Integrations::Openai::ProcessorService` e `GlobalProcessorService` resolvem o `account_language` por `GlobalConfigService.load('DEFAULT_LOCALE', 'english')`, que cai na env porque `DEFAULT_LOCALE` não é um `installation_config` semeado, e interpolam o valor num prompt em inglês: com a env setada o modelo passava a receber "Please respond in pt_BR", que é um código e não uma instrução. Um mapa de código → nome de idioma em `Integrations::OpenaiBaseService.configured_language` cobre os seis idiomas habilitados; `en` resolve para o sentinela `english`, que preserva a instrução genérica e adaptativa. Segue valendo que o idioma do assistente é da instalação inteira, não por conta. O `DEFAULT_LOCALE` também aceita agora a forma com hífen (`pt-BR`), que é a que um guia de deploy escreve; antes ela caía no aviso e a instalação ficava em inglês. **Ambiente:** o valor `DEFAULT_LOCALE=pt_BR` é configuração do deploy, não deste repo: `docker-compose.ecosystem.yml`, `.env.ecosystem.example` e `k8s/base/configmap.yaml` no superprojeto (evolution-ecosystem#114), mais o YAML do box em `public/box/docker-stack.box.yml`, que não vive no superprojeto: ele fica no repo do front e é servido pela tela de licença (evolution-ecosystem-frontend#161). Nos três, só nos serviços do CRM e do seu sidekiq — medido, o auth não lê esta env. Sem ela nada muda neste repo.
- **CRM-583** — A ação `create_pipeline_task` das regras de automação deixa de gravar a tarefa num card sorteado. Ela escolhia o card com `@conversation.pipeline_items.first` — sem ordenação, o que o Rails resolve como `ORDER BY id` sobre uma chave primária uuid. Numa conversa presente em mais de um funil (possível desde que `assign_to_pipeline` parou de apagar os cards dos outros), uma regra `assign_to_pipeline` no funil B seguida de `create_pipeline_task` gravava a tarefa no card do funil A em boa parte das execuções — e, como o autor da tarefa é resolvido a partir de `pipeline.created_by_id`, creditava junto o dono do funil errado. O card passa a ser o do funil que uma ação anterior da mesma regra nomeou (`assign_to_pipeline` ou `update_pipeline_stage`) — inclusive quando essa ação foi um no-op por a conversa já estar no funil, caso em que nenhum card novo nasce e o mais recente apontaria para um funil que a regra nunca citou. Sem ação de funil na regra, vale o card ativo mais recente da conversa; se a ação nomeou um funil que ficou sem card ativo (atribuição recusada), a tarefa não é criada e o skip entra na timeline da regra (`pipeline_target_missing`). Vale para as duas superfícies de execução (regra simples e flow-canvas), que compartilham o handler. ⚠️ **Efeito observável:** conversa cujo único card está `completed` deixa de receber tarefa — antes a tarefa nascia num card já fechado, onde ninguém a via.
- **CRM-489** — Canal WhatsApp do provider 360dialog (`default`) passa a ter a credencial re-sondada pelo job horário de probe, como já acontecia com `whatsapp_cloud` e `notificame`. O probe é um `GET /v1/configs/webhook` read-only (a validação de save continua registrando o webhook via POST) e devolve `:ok` / `:rejected` (401/403) / `:inconclusive` (indisponibilidade ou rate limit do provedor não vira revogação). Com isso o TTL de 7 dias da evidência passa a valer para o 360dialog. ⚠️ **Efeito observável:** canal com chave revogada deixa de ler `connected` para sempre e passa a `error`, disparando o fluxo de reautorização; e, no primeiro deploy, canal cujo carimbo de validação tem mais de 7 dias lê `unknown` até o job alcançá-lo (horário, 25 canais por rodada, nunca sondados primeiro).
